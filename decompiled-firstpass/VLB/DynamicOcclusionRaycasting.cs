...
public class DynamicOcclusionRaycasting : DynamicOcclusionAbstractBase
{
	public struct HitResult
	{
		...
		public bool hasCollider
		{
			get
			{
				if (!(UnityEngine.Object)(object)collider2D)
				{
					return collider3D;
				}
				return true;
		...
		public string name
		{
			get
			{
				...
				if ((bool)(UnityEngine.Object)(object)collider2D)
				{
					return ((UnityEngine.Object)(object)collider2D).name;
				}
		...
		{
			point = ((RaycastHit2D)(ref hit2D)).point;
			normal = ((RaycastHit2D)(ref hit2D)).normal;
	...
	private float raycastMaxDistance => m_Master.raycastDistance * m_RangeMultiplier * m_Master.lossyScale.z;
	...
	{
		return MaterialManager.DynamicOcclusion.ClippingPlane;
	}
	...
	private HitResult GetBestHit2D(Vector3 rayPos, Vector3 rayDir)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			if ((considerTriggers || !((RaycastHit2D)(ref array[i])).collider.isTrigger) && ((Component)(object)((RaycastHit2D)(ref array[i])).collider).gameObject != m_Master.gameObject && ((RaycastHit2D)(ref array[i])).collider.bounds.GetMaxArea2D() >= minOccluderArea && ((RaycastHit2D)(ref array[i])).distance < num2)
			{
	...
	protected override void OnModifyMaterialCallback(MaterialModifier.Interface owner)
	{
		...
		owner.SetMaterialProp(ShaderProperties.DynamicOcclusionClippingPlaneWS, new Vector4(plane.normal.x, plane.normal.y, plane.normal.z, plane.distance));
