...
public class StandardUIQuestTracker : MonoBehaviour
{
	...
	{
		if (m_started && !(DialogueManager.instance == null) && !(GetComponentInParent<DialogueSystemController>() != null))
		{
	...
	{
		if (m_started && !(DialogueManager.instance == null))
		{
			DialogueManager.instance.receivedUpdateTracker -= UpdateTracker;
		}
	}
	...
	{
		isVisible = true;
		if (!string.IsNullOrEmpty(playerPrefsToggleKey))
		{
	...
	protected virtual IEnumerator RefreshAtEndOfFrame()
	{
		...
		int num = 0;
		string[] allQuests = QuestLog.GetAllQuests((QuestState)((showActiveQuests ? 66 : 0) | (showCompletedQuests ? 12 : 0)));
	...
	protected virtual void AddQuestTrack(string quest)
	{
		...
		string questHeading = GetQuestHeading(quest);
		...
		gameObject.name = questHeading;
		gameObject.transform.SetParent(container.transform, worldPositionStays: false);
		gameObject.SetActive(value: true);
		StandardUIQuestTrackTemplate component = gameObject.GetComponent<StandardUIQuestTrackTemplate>();
		instantiatedItems.Add(component);
		if (component != null)
		{
			SetupQuestTrackInstance(component, quest, questHeading);
	...
	{
		return FormattedText.Parse((questDescriptionSource == QuestDescriptionSource.Title) ? QuestLog.GetQuestTitle(quest) : QuestLog.GetQuestDescription(quest), DialogueManager.masterDatabase.emphasisSettings).text;
	}
	...
	protected virtual void SetupQuestTrackInstance(StandardUIQuestTrackTemplate questTrack, string quest, string heading)
	{
		...
		for (int i = 1; i <= questEntryCount; i++)
		{
			...
			string text = FormattedText.Parse(GetQuestEntryText(quest, i, questEntryState), DialogueManager.masterDatabase.emphasisSettings).text;
			if (!string.IsNullOrEmpty(text))
			{
				questTrack.AddEntryDescription(text, questEntryState);
			}
		}
	}
