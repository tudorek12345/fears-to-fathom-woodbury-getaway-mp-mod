...
public class StandardUIInputField : UIPanel, ITextFieldUI
{
	...
	protected override void Update()
	{
		...
		{
			if (InputDeviceManager.IsKeyDown(acceptKey) || InputDeviceManager.IsButtonDown(acceptButton) || (m_touchScreenKeyboard != null && m_touchScreenKeyboard.status == TouchScreenKeyboard.Status.Done))
			{
				AcceptTextInput();
			}
			else if (InputDeviceManager.IsKeyDown(cancelKey) || InputDeviceManager.IsButtonDown(cancelButton) || (m_touchScreenKeyboard != null && m_touchScreenKeyboard.status == TouchScreenKeyboard.Status.Canceled))
			{
				CancelTextInput();
			}
		}
	...
	{
		if (!allowBlankInput && string.IsNullOrEmpty(inputField.text))
		{
			return;
		}
		m_isAwaitingInput = false;
	...
	protected virtual void Show()
	{
		...
		if (inputField != null)
		{
			...
			if ((Object)(object)base.eventSystem != null)
			{
	...
	protected virtual void SetActive(bool value)
	{
		...
		{
			((Component)(object)panel).gameObject.SetActive(value);
		}
		if ((Object)(object)panel == null || value)
		{
			if (label != null)
			{
