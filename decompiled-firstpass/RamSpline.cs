...
public class RamSpline : MonoBehaviour
{
		...
		for (int k = 0; k < pointsDown.Count; k++)
		{
			...
			for (int l = 0; l < vertsInShape; l++)
			{
				...
				else
				{
					...
					{
						val = hitInfo.collider.gameObject.GetComponent<Terrain>();
						if ((bool)(UnityEngine.Object)(object)val && !terrainsUnder.Contains(val))
						{
							terrainsUnder.Add(val);
						}
	...
	public void TerrainCarve(Terrain singleTerrain = null)
	{
		...
		{
			if ((UnityEngine.Object)(object)singleTerrain != null && (UnityEngine.Object)(object)val != (UnityEngine.Object)(object)singleTerrain)
			{
				continue;
			}
			TerrainData terrainData = val.terrainData;
			float y = ((Component)(object)val).transform.position.y;
			float x = val.terrainData.size.x;
			float y2 = val.terrainData.size.y;
			float z = val.terrainData.size.z;
			...
			for (num4 = 0; num4 < pointsUp.Count; num4 = Mathf.Clamp(num4 + num3 - 1, 0, pointsUp.Count))
			{
				...
				num6 -= ((Component)(object)val).transform.position.x + distSmooth;
				num7 -= ((Component)(object)val).transform.position.x - distSmooth;
				num8 -= ((Component)(object)val).transform.position.z + distSmooth;
				num9 -= ((Component)(object)val).transform.position.z - distSmooth;
				num6 *= num2;
				num7 *= num2;
				num8 *= num;
				num9 *= num;
				...
				for (int m = 0; m < heights.GetLength(0); m++)
				{
					...
					{
						zero.x = ((float)n + num6) / num2 + ((Component)(object)val).transform.position.x;
						zero.z = ((float)m + num8) / num + ((Component)(object)val).transform.position.z;
	...
	public void TerrainPaintMeshBased(Terrain singleTerrain = null)
	{
		...
		foreach (Terrain val in activeTerrains)
		{
			...
			for (num4 = 0; num4 < pointsUp.Count; num4 = Mathf.Clamp(num4 + num3 - 1, 0, pointsUp.Count))
			{
				...
				{
					Debug.LogWarning("RAM: Terrain \"" + ((UnityEngine.Object)(object)val).name + "\" doesn't have layer: " + currentSplatMap);
					break;
				}
				Vector3 zero = Vector3.zero;
				_ = Vector3.zero;
	...
	public void TerrainClearFoliage(bool details = true)
	{
		...
		{
			TerrainData terrainData = val.terrainData;
			Transform transform = ((Component)(object)val).transform;
			_ = ((Component)(object)val).transform.position;
			float x = val.terrainData.size.x;
			_ = val.terrainData.size;
			float z = val.terrainData.size.z;
					...
					if (num7 - num6 > 0f && num9 - num8 > 0f)
					{
						for (int m = 0; m < terrainData.detailPrototypes.Length; m++)
						{
							...
							for (int n = 0; n < detailLayer.GetLength(0); n++)
							{
								...
								{
									zero.x = ((float)num10 + num6 + 0.5f) / num2 + ((Component)(object)val).transform.position.x;
									zero.z = ((float)n + num8 + 0.5f) / num + ((Component)(object)val).transform.position.z;
									Ray ray = new Ray(zero + Vector3.up * 3000f, Vector3.down);
			...
			else
			{
				...
				foreach (TreeInstance val2 in treeInstances)
				{
					zero.x = val2.position.x * x + transform.position.x;
					zero.z = val2.position.z * z + transform.position.z;
