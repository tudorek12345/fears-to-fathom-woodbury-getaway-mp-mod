...
public class LakePolygon : MonoBehaviour
{
	...
	public void TerrainCarve(bool terrainShow = false)
	{
		...
		foreach (Terrain val in array)
		{
			...
			float y2 = ((Component)(object)val).transform.position.y;
			float x = val.terrainData.size.x;
			float y3 = val.terrainData.size.y;
			...
			if (lakePolygonCarveData == null || distSmooth != lakePolygonCarveData.distSmooth)
			{
				...
				num -= ((Component)(object)val).transform.position.x + distSmooth;
				num2 -= ((Component)(object)val).transform.position.x - distSmooth;
				num3 -= ((Component)(object)val).transform.position.z + distSmooth;
				num4 -= ((Component)(object)val).transform.position.z - distSmooth;
				num *= num6;
				num2 *= num6;
				num3 *= num5;
				num4 *= num5;
	...
	public void TerrainPaint(bool terrainShow = false)
	{
		...
		foreach (Terrain val in array)
		{
			...
			if (lakePolygonPaintData == null || num != lakePolygonPaintData.distSmooth)
			{
				...
				Debug.Log(num7 + " " + num8);
				num3 -= ((Component)(object)val).transform.position.x + num;
				num4 -= ((Component)(object)val).transform.position.x - num;
				num5 -= ((Component)(object)val).transform.position.z + num;
				num6 -= ((Component)(object)val).transform.position.z - num;
				num3 *= num8;
				num4 *= num8;
				num5 *= num7;
				num6 *= num7;
					...
					for (int l = 0; l < alphamaps.GetLength(1); l++)
					{
						...
						if (meshCollider.Raycast(ray, out var hitInfo, 10000f))
						{
							...
							if (addCliffSplatMap)
							{
								...
								if (((Collider)(object)((Component)(object)val).GetComponent<TerrainCollider>()).Raycast(ray, out hitInfo, 10000f))
								{
	...
	public void TerrainClearTrees(bool details = true)
	{
		...
		foreach (Terrain val in array)
		{
			...
			num -= ((Component)(object)val).transform.position.x + distanceClearFoliage;
			num2 -= ((Component)(object)val).transform.position.x - distanceClearFoliage;
			num3 -= ((Component)(object)val).transform.position.z + distanceClearFoliage;
			num4 -= ((Component)(object)val).transform.position.z - distanceClearFoliage;
			num *= num6;
			num2 *= num6;
			num3 *= num5;
			num4 *= num5;
			...
			if (!details)
			{
				...
				foreach (TreeInstance val2 in array3)
				{
